"""Round16 matched recipe/budget evaluation using unchanged MPPI and physical scoring."""
...
RECIPES = [f"{r}_{b}" for r in ["dense", "recorded", "log", "mixed"] for b in ["short", "long"]]
CELLS = RECIPES + ["original"]
                        ...
                        for e in range(10):
                            out.append(
                                dict(
                                    ...
                                    coverage=coverage,
                                    arm=arm,
                                    model=model_name(d, coverage, arm),
                                    model_seed=seed,
                                    wind=wind,
                                    episode=e,
                                    environment_seed=710000 + e,
                                    trajectory_seed=720000 + e,
                                    planning_seed=730000 + e,
                                )
                            )
    return out
...
def fresh_anchor(index):
    wind = WINDS[index // 10]
    e = index % 10
    sim = QuadrotorSim(seed=710000 + e)
    traj = random_trajectory(np.random.default_rng(720000 + e), 33.02, 0.02)
    ...
    bank = diag.TraceMPPI(None, n_samples=64, rng=np.random.default_rng(730000 + e)).sample_bank(
        nominal
    )
    return (
        snap,
...
def model_name(d, coverage, arm):
    if coverage == "original":
        return f"r11b_d{d}_{arm}_r0"
    recipe, budget = coverage.split("_")
    if budget == "short" and recipe in ["log", "mixed"]:
        return f"r15_d{d}_{recipe}_{arm}"
    return f"r16_d{d}_{recipe}_{budget}_{arm}"


def model_path(d, coverage, arm, seed):
    family = (
        "round11b"
        if coverage == "original"
        else ("round15_coverage" if coverage in ["log_short", "mixed_short"] else "round16_bridge")
    )
    return f"{family}_data{d}_v1/{model_name(d, coverage, arm)}_seed{seed}/checkpoint_best.pt"


def make_manifest(root):
    models, accepted = [], {}
    original_path = ROOT / "runs/evidence/round13_bundle/formal.json"
    original = json.loads(original_path.read_text())["checkpoint_manifest"]["checkpoints"]
    accepted[str(original_path.relative_to(ROOT))] = sha(original_path)
    original_hashes = {x["checkpoint_relpath"]: x["checkpoint_sha256"] for x in original}
    for d in range(5):
        gates = {}
        for round_id, family, count in [(15, "coverage", 12), (16, "bridge", 36)]:
            ap = (
                ROOT
                / f"runs/evidence/round{round_id}_bundle/round{round_id}_{family}_data{d}_acceptance.json"
            )
            a = json.loads(ap.read_text())
            assert a["accepted_runs"] == count and a["accepted_npz"] == count * 2 and not a["pilot"]
            accepted[str(ap.relative_to(ROOT))] = sha(ap)
            gates[round_id] = a
        for coverage in CELLS:
            for arm in ["frozen", "updated"]:
                for seed in [70, 71, 72]:
                    rel = model_path(d, coverage, arm, seed)
                    path = Path(root) / rel
                    digest = sha(path)
                    cp = torch.load(path, map_location="cpu", weights_only=False)
                    if coverage == "original":
                        assert digest == original_hashes[rel]
                    else:
                        gate = gates[15 if coverage in ["log_short", "mixed_short"] else 16]
                        assert digest == gate["sha256"]["runs/results/" + rel]
                        assert cp["provenance"]["git_commit"] == gate["source"]
                    assert cp["config"]["model"]["name"] == model_name(d, coverage, arm)
                    assert cp["config"]["seed"] == seed
                    models.append(
                        dict(
                            dataset=d,
                            coverage=coverage,
    ...
    rr = rows()
    return dict(
        schema="round16-evaluation-v1",
        models=models,
        rows=rr,
        source_sha256=sources(),
        protocol_sha256={
            n: sha(ROOT / "docs" / n) for n in ["ROUND16_PROTOCOL.md", "ROUND16_EVALUATION.md"]
        },
        training_acceptance=accepted,
        query_pilot=[0, 10, 20, 30],
        control_pilot=[
            r["index"]
            for r in rr
...
def validate(m):
    assert (
        m["schema"] == "round16-evaluation-v1"
    ...
    assert m["query_pilot"] == [0, 10, 20, 30]
    assert m["control_pilot"] == [
        r["index"]
        for r in rows()
        if r["dataset_replicate"] == 0 and r["model_seed"] == 70 and r["episode"] == 0
    ]
    for x in m["models"]:
        assert x["path"] == model_path(x["dataset"], x["coverage"], x["arm"], x["seed"])
