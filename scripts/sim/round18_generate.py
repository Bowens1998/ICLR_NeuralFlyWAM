"""Fixed crossed-trajectory Round18 confirmation generation; never overwrites a flight.
...
CONDITIONS = [("nowind", 0.0), ("10wind", 1.3), ("20wind", 2.5), ("40wind", 4.9)]
    ...
    for replicate in range(5):
        for group, ntraj, conditions in [
            ...
            for j in range(ntraj):
                trajectory_seed = 1810000 + replicate * 1000 + offset + j
                noise_seed = 1820000 + replicate * 1000 + offset + j
                for condition, mean in conditions:
                    rows.append(
                        dict(
                            ...
                            name=f"r18d{replicate}_t{trajectory_seed}_{group}_{condition}",
                            condition=condition,
                            mean_wind=mean,
                            wind_amplitude=2.4 if group in ("slow", "fast") else 0.0,
                            wind_angular_frequency={"slow": 0.25, "fast": 1.0}.get(group, 0.0),
                            duration_seconds=60.0,
                        )
                    )
    return dict(
        protocol="docs/ROUND18_PROTOCOL.md",
        params=asdict(SimParams()),
...
def main():
    ...
    ap.add_argument("--design", type=Path, default=REPO / "configs/sim/round18_generation.json")
