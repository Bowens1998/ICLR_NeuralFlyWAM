...
def design():
    old = json.loads((ROOT / "configs/sim/round11_generation.json").read_text())
    rows = []
    for d in range(5):
    ...
    assert len(rows) == 80
    return dict(
        schema="round15-branches-v1",
        params=old["params"],
        rows=rows,
        protocol_sha256=sha(ROOT / "docs/ROUND15_PROTOCOL.md"),
    )
...
def generate(m, index, data_root, out):
    ...
    for center, snap in snapshots.items():
        seed = [15, spec["dataset"], row["index"], center]
