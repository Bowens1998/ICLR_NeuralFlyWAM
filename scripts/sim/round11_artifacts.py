"""Build new whole-flight Round11B manifests after complete generation acceptance."""
...
SOURCE = '02eedf84d877d646f0a378ad3b403f083a5c856a'
GROUPS = {'train': 'd1_train', 'val': 'd1_val', 'static': 'd2_static_ood',
          'slow': 'd3_changing_ood', 'fast': 'd3_changing_ood'}
...
def make_manifest(roster, evidence, replicate, root):
    ...
    for row in rows:
        splits[GROUPS[row['group']]].append(dict(
            file=row['name'], condition=row['condition'],
            trajectory=f"t{row['trajectory_seed']}",
            sha256=evidence[row['index']]['csv_sha256'], start=100, stop=2950))
    splits['d0_smoke'] = [{**splits['d1_train'][0], 'stop': 356}]
    m = dict(data_root=str(root), sample_rate_hz=50, dt=.02,
             history_seconds=2., horizon_seconds=1., history_steps=100,
             horizon_steps=50, decimate=1, static_ood_conditions=['30wind', '50wind', '70wind'],
             transfer_rate_mode='backward_causal_v2', splits=splits,
             dataset_replicate=replicate, generation_source=SOURCE,
...
def accept(root, roster):
    ...
    assert {p.name.removesuffix('.generation.json') for p in root.glob('*.generation.json')} == expected
    evidence = {}
    for row in roster['rows']:
        ...
        trajectory = random_trajectory(np.random.default_rng(row['trajectory_seed']), 60., .02)
        ...
        f = load_flight(path)
        assert f.n_steps == 3000 and abs(f.dt - .02) < 1e-8
...
def main():
    ...
    roster = json.loads((REPO / 'configs/sim/round11_generation.json').read_text())
    ...
    for replicate in range(5):
        ...
        norm.save(norm_path)
        records.append(dict(replicate=replicate, split_checksum=manifest['checksum'],
                            normalizer_sha256=file_sha256(norm_path),
                            train_flights=len(ds.blocks), train_windows=len(ds)))
    (args.output / 'acceptance.json').write_text(json.dumps(dict(
        source=SOURCE, accepted_flights=len(evidence), replicates=records), indent=2) + '\n')
