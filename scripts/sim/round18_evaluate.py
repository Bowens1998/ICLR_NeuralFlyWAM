"""Round18 matched recipe/budget evaluation using unchanged MPPI and physical scoring."""
...
RECIPES = [f"{r}_{b}" for r in ["log", "mixed"] for b in ["short", "long"]]
CELLS = RECIPES
                        ...
                        for e in range(10):
                            out.append(
                                dict(
                                    ...
                                    wind=wind,
                                    episode=e,
                                    environment_seed=1830000 + e,
                                    trajectory_seed=1840000 + e,
                                    planning_seed=1850000 + e,
                                )
                            )
    return out
...
def fresh_anchor(index):
    wind = WINDS[index // 10]
    e = index % 10
    sim = QuadrotorSim(seed=1830000 + e)
    traj = random_trajectory(np.random.default_rng(1840000 + e), 33.02, 0.02)
    ...
    bank = diag.TraceMPPI(None, n_samples=64, rng=np.random.default_rng(1850000 + e)).sample_bank(
        nominal
    )
    return (
        snap,
...
def model_name(d, coverage, arm):
    return f"r18_d{d}_{coverage}_{arm}"


def model_path(d, coverage, arm, seed):
    return (
        f"round18_confirm_data{d}_v1/{model_name(d, coverage, arm)}_seed{seed}/checkpoint_best.pt"
    )
...
def make_manifest(root):
    models, accepted = [], {}
    for d in range(5):
        ap = ROOT / f"runs/evidence/round18_bundle/round18_confirm_data{d}_acceptance.json"
        gate = json.loads(ap.read_text())
        assert gate["accepted_runs"] == 24 and gate["accepted_npz"] == 48 and not gate["pilot"]
    ...
    rr = rows()
    return dict(
        schema="round18-evaluation-v1",
        models=models,
        rows=rr,
        source_sha256=sources(),
        protocol_sha256={
            n: sha(ROOT / "docs" / n) for n in ["ROUND18_PROTOCOL.md", "ROUND18_EVALUATION.md"]
        },
...
def validate(m):
    assert (
        m["schema"] == "round18-evaluation-v1"
