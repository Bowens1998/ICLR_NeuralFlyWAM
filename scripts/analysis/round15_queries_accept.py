...
import round15_evaluate as runner
...
def accept(path, manifest, source, pilot=False):
    ...
    assert r["panel"] == "query" and r["index"] in range(30)
    ...
    assert r["anchor"]["noise_seed_words"] == {
        phase: D.protocol.noise_seed_words("a1", 150000 + r["index"], 100, phase)
    ...
    with np.load(trace, allow_pickle=False) as z:
        ...
        for phase in ["construct", "evaluate"]:
            noise = D.noise_bundle(
                D.protocol.noise_seed_words("a1", 150000 + r["index"], 100, phase), 8
            )
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    indices = [0, 10, 20] if a.pilot else range(30)
    records = [
