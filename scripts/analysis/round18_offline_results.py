"""Original static logged-action prediction audit; all 120 matched models retained."""
...
import numpy as np
from round18_results import RECIPES, summarize
...
def main():
    ...
    for d in range(5):
        accepted = {}
        for n, family, count in [(18, "confirm", 24)]:
        ...
        for c in cells:
            ...
            for arm in ["frozen", "updated"]:
                for seed in [70, 71, 72]:
                    name = f"r18_d{d}_{recipe}_{budget}_{arm}"
    ...
    a.output.write_text(
        json.dumps(
            dict(
                ...
                records=records,
                summary=summaries,
                caveat="Static logged tests are 3.7/6.1/8.5, not the feedback zero/4.9 boundary conditions. n=5 training datasets, seed/window averages within dataset, descriptive nonsimultaneous intervals. Full120matched models, no outcome selection.",
            ),
            indent=2,
        )
        + "\n"
