...
CELLS = [f"{r}_{b}" for r in RECIPES for b in ["short", "long"]] + ["original"]
...
def summarize(means, cells):
    ...
    for b in ["short", "long"]:
        for left, right in [("recorded", "dense"), ("log", "recorded"), ("mixed", "log")]:
            key = f"{b}/{left}-{right}"
...
def main():
    ...
    expected = 40 if a.panel == "query" else 10800
    ...
    result = dict(
        ...
        records=records,
        caveat="Result-informed follow-up; n=5 training datasets; descriptive nonsimultaneous t95 intervals, no p-values. Budget includes schedule length/selection opportunities. Conditional contrasts do not identify a unique latent mechanism.",
    )
